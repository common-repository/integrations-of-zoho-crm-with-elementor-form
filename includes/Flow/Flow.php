...
final class Flow
{
    ...
    public function get($data)
    {
        ...
        if (!is_null($missing_field)) {
            wp_send_json_error(sprintf(__('%s can\'t be empty', 'izcrmef'), $missing_field));
        }
        $integrationHandler = new FlowController();
        $integrations = $integrationHandler->get(
            ['id' => $data->id],
    ...
    public function save($data)
    {
        ...
        wp_send_json_success(['id' => $saveStatus, 'msg' => __('Integration saved successfully', 'izcrmef')]);
    }
    ...
    public function flowClone($data)
    {
        ...
        if (!is_null($missingId)) {
            wp_send_json_error(sprintf(__('%s can\'t be empty', 'izcrmef'), $missingId));
        }
        $integrationHandler = new FlowController();
        $integrations = $integrationHandler->get(
            ['id' => $data->id],
        ...
        if (!is_wp_error($integrations) && count($integrations) > 0) {
            ...
            wp_send_json_error(__('Flow ID is not exists', 'izcrmef'));
        }
    }
    ...
    public function update($data)
    {
        ...
        wp_send_json_success(__('Integration updated successfully', 'izcrmef'));
    }
    ...
    public function delete($data)
    {
        ...
        if (!is_null($missing_field)) {
            wp_send_json_error(sprintf(__('%s cann\'t be empty', 'izcrmef'), $missing_field));
        }
        $integrationHandler = new FlowController();
        ...
        wp_send_json_success(__('Integration deleted successfully', 'izcrmef'));
    }

    public function bulkDelete($param)
    {
        if (!is_array($param->flowID) || $param->flowID === []) {
            wp_send_json_error(sprintf(__('%s cann\'t be empty', 'izcrmef'), 'Integration id'));
        }
    ...
    public function toggle_status($data)
    {
        ...
        wp_send_json_success(__('Status changed successfully', 'izcrmef'));
    }
        ...
        if (!is_wp_error($flows) && !empty($flows)) {
            ...
            foreach ($flows as $flowData) {
                ...
                if (
                    ...
                    $error = new WP_Error('Conditional Logic False', __('Conditional Logic not matched', 'izcrmef'));
