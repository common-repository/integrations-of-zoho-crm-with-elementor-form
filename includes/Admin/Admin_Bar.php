...
class Admin_Bar
{
    ...
    public function AdminAssets($current_screen)
    {
        ...
        if (strpos($current_screen, 'izcrmef') === false) {
            return;
        }
        ...
        $prefix = 'FITEZCRM';
        if (is_readable(IZCRMEF_PLUGIN_DIR_PATH . DIRECTORY_SEPARATOR . 'port')) {
            $devPort = file_get_contents(IZCRMEF_PLUGIN_DIR_PATH . DIRECTORY_SEPARATOR . 'port');
            ...
            wp_enqueue_script(
                'index-' . $prefix . '-MODULE',
                IZCRMEF_ASSET_URI . "/index-" . IZCRMEF_VERSION . ".js",
                [],
                null
            );
        }
        ...
        $izcrmef = apply_filters(
            ...
            [
                ...
                'baseURL'    => $base_path_admin . 'admin.php?page=izcrmef#',
                'siteURL'    => site_url(),
                ...
                'userMail'   => $userMail,
                'redirect' => get_rest_url() . 'izcrmef/redirect',
            ]
        );
        ...
        wp_localize_script('index-' . $prefix . '-MODULE', 'izcrmef', $izcrmef);
    ...
    public function RemoveAdminNotices()
    {
        ...
        if (empty($plugin_page) || strpos($plugin_page, 'izcrmef') === false) {
            return;
        }
