...
class Model
{
    ...
    {
        if (is_null($data)) {
            return new WP_Error('empty_data', __('Form data is empty', 'izcrmef'));
        }
    ...
    public function count($condition = null)
    {
        ...
        if (!$result) {
            ...
            return new WP_Error('db_error', __('Result is empty', 'izcrmef'));
        } else {
    ...
    public function duplicate(array $columns, array $duplicate, array $condition)
    {
        if (!(!\is_null($columns)
            ...
            return new WP_Error(
                'duplicate_error',
                __('Nothing to duplicate', 'izcrmef')
            );
        }
    ...
    public function trash(array $condition = null)
    {
        if (
            ...
            return new WP_Error(
                'deletion_error',
                __('At least 1 condition needed', 'izcrmef')
            );
        }
        $update_condition = (!\is_null($condition) &&
            array_keys($condition) !== range(0, count($condition) - 1)) ? $condition : null;
    ...
    protected function getResult($db_response = null)
    {
        ...
        if (!$db_response) {
            ...
            $response = new WP_Error('result_empty', __('Result is empty', 'izcrmef'));
